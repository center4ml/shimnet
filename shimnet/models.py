...
class ConvDecoder(torch.nn.Module):
    def __init__(self, input_dim=None, hidden_dim=64, output_dim=1, dropout=0, kernel_size=7, activation="relu"):
        super().__init__()
        ...
        layers = [
            ...
            get_activation(activation),
            torch.nn.Dropout(dropout),
            torch.nn.ConvTranspose1d(hidden_dim, output_dim, kernel_size),
            get_activation(activation),
            torch.nn.Dropout(dropout),
        ]
        self.net = torch.nn.Sequential(*layers)
